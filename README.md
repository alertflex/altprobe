...
* Active response (blocking of a suspicious source or destination IP via firewalld) based on the local collector filtering policies or based on a remote message from the controller if IP match to IOC from MISP DB. Now, an on-premises node can work as a security gateway.
